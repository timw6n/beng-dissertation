...
% New commands to workaround the fact that \section* and \subsection* cannot be embedded in Markdown files
\newcommand{\usection}[1]{\section*{#1}}
\newcommand{\usubsection}[1]{\subsection*{#1}}

$if(title)$
\title{{\large Submitted in part fulfilment for the degree of BEng}\\\vspace{1em}$title$$if(subtitle)$\\\vspace{0.5em}{\large $subtitle$}$endif$}
$endif$
$if(author)$
\author{$for(author)$$author$$sep$ \and $endfor$ \\\vspace{0.5em} Supervisor: $supervisor$ }
$endif$
\date{$date$}
$for(header-includes)$
$header-includes$
